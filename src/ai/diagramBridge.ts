...
export function blockToNode(b: DiagramBlock): AINode {
  return { id: b.id, type: b.type, x: b.x, y: b.y, width: b.width, height: b.height, label: b.text };
}
...
function makeArrow(id: string, source: DiagramBlock, target: DiagramBlock, label?: string): ArrowElement {
  ...
  for (const op of ops) {
    switch (op.type) {
      ...
      case "addEdge": {
        ...
        if (source && target) {
          arrows = [...arrows.filter((a) => a.id !== op.edge.id), makeArrow(op.edge.id, source, target, op.edge.label)];
        } else {
      ...
      case "updateEdge": {
        arrows = arrows.map((a) => {
          ...
          if (op.patch.source || op.patch.target) {
            ...
            if (src && tgt) return makeArrow(a.id, src, tgt, label);
          }
...
/** 미리보기용: operations 를 사람이 읽을 한 줄 요약으로. */
export function summarizeOperation(op: Operation): string {
  switch (op.type) {
    case "addNode":
      return `+ 블록 추가: "${op.node.label}" (${op.node.type})`;
    case "updateNode":
      return `~ 블록 수정: ${op.id}${op.patch.label ? ` → "${op.patch.label}"` : ""}`;
    case "deleteNode":
      return `- 블록 삭제: ${op.id}`;
    case "moveNode":
      return `→ 블록 이동: ${op.id} (${op.x}, ${op.y})`;
    case "addEdge":
      return `+ 연결 추가: ${op.edge.source} → ${op.edge.target}${op.edge.label ? ` "${op.edge.label}"` : ""}`;
    case "updateEdge":
      return `~ 연결 수정: ${op.id}`;
    case "deleteEdge":
      return `- 연결 삭제: ${op.id}`;
    case "layoutDiagram":
      return `⟲ 자동 정렬 (${op.direction ?? "LR"})`;
  }
}
